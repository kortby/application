    ...
    protected function produce_output($columns, $iTotal, $iFilteredTotal, $rResult, $custom_columns = '')
    {
      ...
      foreach($rResult->result_array() as $row_key => $row_val)
      {
        ...
        {        	
                 ...
                 switch ($col_key) {
                    /*case "published":
                        $row_val['published'] == 1 ? $activep = TRUE : $activep = FALSE;
                        $aaData[$row_key][] = form_hidden('hpulished', $row_val['id_new']) . form_checkbox('published', $activep, $activep, 'class=published');
                        break;
                    case "archived":
                        $row_val['archived'] == 1 ? $activea = TRUE : $activea = FALSE;
                        $aaData[$row_key][] = form_hidden('hpulished', $row_val['id_new']) . form_checkbox('archived', $activea, $activea, 'class=archived');
                        break;
                    /*case "ambassade":
                        //$row_val['archived'] == 1 ? $activea = TRUE : $activea = FALSE;
                        //$aaData[$row_key][] = form_hidden('hpulished', $row_val['id_ambassade']) . form_checkbox('archived', true, true, 'class=archived');
                        $aaData[$row_key][] = '<span id="' . $row_val['id_ambassade'] . '" class="ui-icon ui-icon-circle-close delete0_ambassade"></span>' . $row_val['ambassade'];
                        //$aaData[$row_key][] = $row_val['created'];
                        break;*/ 
                    default:
